...
# Jellyfin
JELLYFIN_PUBLISHED_SERVER_URL=$jellyfin_url
...
step_completion() {
    ...
    local dashboard_url="https://dashboard.$DOMAIN"
    local jellyfin_link="https://jellyfin.$DOMAIN"
    if [ "$ACCESS_MODE" = "local" ]; then
        dashboard_url="http://localhost:7575"
        jellyfin_link="http://localhost:8096"
    fi

    echo -e "${COLORS[BLUE]}${SYMBOLS[GLOBE]} ${COLORS[BOLD]}Access your services:${COLORS[RESET]}"
    echo -e "┌────────────────────────────────────────────────────────────────────────┐"
    echo -e "│ ${SYMBOLS[MOVIE]} ${COLORS[BOLD]}Main Dashboard:${COLORS[RESET]} ${COLORS[CYAN]}$dashboard_url${COLORS[RESET]}              │"
    echo -e "│ ${SYMBOLS[TV]} ${COLORS[BOLD]}Watch Movies/TV:${COLORS[RESET]} ${COLORS[CYAN]}$jellyfin_link${COLORS[RESET]}             │"
    if [ "$ACCESS_MODE" != "local" ]; then
...
main() {
    ...
    case $CURRENT_STEP in
        1|2) step_access_mode ;;
    esac

    case $CURRENT_STEP in
        2|3) step_system_check ;;
    esac

    case $CURRENT_STEP in
        3|4) step_domain_setup ;;
    esac

    case $CURRENT_STEP in
        4|5) step_cloudflare_setup ;;
    esac

    case $CURRENT_STEP in
        5|6) step_storage_setup ;;
    esac

    case $CURRENT_STEP in
        6|7) step_environment_config ;;
    esac

    case $CURRENT_STEP in
        7|8) step_gpu_detection ;;
    esac
    ...
    case $CURRENT_STEP in
        9|10) step_dns_configuration ;;
    esac

    case $CURRENT_STEP in
        10|11) step_port_forwarding ;;
    esac

    case $CURRENT_STEP in
        11|12) step_service_configuration ;;
    esac

    case $CURRENT_STEP in
        12|13) step_completion ;;
    esac
